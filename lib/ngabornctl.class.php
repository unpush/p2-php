...
class NgAbornCtl{
	...
	function loadNgAborns()
	{
		...
		// expack:
		$ngaborns['aborn_chain'] = 0;	// 連鎖あぼ～ん ($_exconf['aborn']['chain_aborn_ng'] == 0)
		$ngaborns['ng_aa'] = 0;	// AA
		$ngaborns['ng_aborn'] = 0;	// 連鎖あぼ～ん ($_exconf['aborn']['chain_aborn_ng'] == 1)
		$ngaborns['ng_chain'] = 0;	// 連鎖NG
	...
	function readNgAbornFromFile($filename)
	{
		...
		if (file_exists($array['file']) && $lines = @file($array['file'])) {
