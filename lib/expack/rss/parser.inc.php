...
require_once 'XML/RSS2.php'; // PEAR::XML_RSS を継承したクラス

if ($GLOBALS['_exconf']['rss']['with_imgcache'] &&
	((!$GLOBALS['_conf']['ktai'] && $GLOBALS['_exconf']['imgCache']['*'] % 2 == 1) ||
	($GLOBALS['_conf']['ktai'] && $GLOBALS['_exconf']['imgCache']['*'] >= 2))
) {
...
{
	global $_conf, $_exconf, $_info_msg_ht, $datdir;
	...
	if (!file_exists($localpath) || $refresh ||
		filemtime($localpath) < (time() - $_exconf['rss']['check_interval'] * 60)
	) {
		$dl = &P2Util::fileDownload($remotefile, $localpath);
		if ($dl->is_success()) {
			chmod($localpath, $_conf['rss_perm']);
		}
	}
