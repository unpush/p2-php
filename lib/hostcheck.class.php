...
EOF;
    ...
    function getHostAuth()
    {
        ...
        if (
            ...
            ( $flag == $_HOSTCHKCONF['host_type']['emobile'] && HostCheck::isAddremobile() ) ||
	 ...
	 * emobile?
	 *
	 * emobileは公式サイトで公表していないので以下より
	 * @link http://whois.nic.ad.jp/cgi-bin/whois_gw?key=60.254.192.0/18
	 */
	function isAddremobile($addr = null)
	{
		if (is_null($addr)) {
			$addr = $_SERVER['REMOTE_ADDR'];
		}
		$emHost = '/^eM(\d|-)*\.pool\.emnet\.ne\.jp$/';
		$emBand = array(
