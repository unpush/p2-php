...
class DB_sqlite3 extends DB_common
{
     ...
     * require_once 'DB.php';
     * 
     * $dsn = 'sqlite3:///path/and/name/of/db/file';
     * $options = array(
    ...
    function connect($dsn, $persistent = false)
    {
        ...
        if (!$dsn['database']) {
            return $this->sqlite3RaiseError(DB_ERROR_ACCESS_VIOLATION);
        }

        if ($dsn['database'] !== ':memory:') {
            if (!file_exists($dsn['database'])) {
                if (!touch($dsn['database'])) {
                    return $this->sqlite3RaiseError(DB_ERROR_NOT_FOUND);
                }
                if (!isset($dsn['mode']) ||
                    !is_numeric($dsn['mode']))
                {
            ...
            if (!is_file($dsn['database'])) {
                return $this->sqlite3RaiseError(DB_ERROR_INVALID);
            }
        ...
        $this->connection = @new SQLite3($dsn['database']);
    ...
    function simpleQuery($query)
    {
        ...
        $this->result = $result;
        if (!$this->result) {
            return $this->sqlite3RaiseError(null);
        }

        return $ismanip ? DB_OK : $this->result;
    }
     ...
     */
    function fetchInto(SQLite3_result $result, &$arr, $fetchmode, $rownum = null)
    {
        if ($rownum !== null) {
     ...
     * because DB_result is a separate object.
     *
     * @param object $result  PHP's query result object
     *
     * @return int  the number of columns.  A DB_Error object on failure.
     *
     * @see DB_result::numCols()
     */
    function numCols(SQLite3_result $result)
    {
        $cols = @$result->numColumns();
        if (!$cols) {
            return $this->sqlite3RaiseError();
        }
        return $cols;
    }
     ...
     */
    function numRows(SQLite3_result $result)
    {
    ...
    {
        $connection = $this->connection ? $this->connection : new SQLite3(':memory:');
        return @$connection->escapeString($str);
    }
     ...
     */
    function sqlite3RaiseError($errno = null)
    {
    ...
    {
        return $this->_lasterror ? $this->_lasterror
            : ($this->connection ? $this->connection->lastErrorMsg() : '');
    }
    ...
    function errorCode($errormsg)
    {
        ...
        $errormsg = preg_replace('/^SQLite3[A-Za-z_]*::[A-Za-z_]+\(\)[^:]*: /i', '', $errormsg);
    ...
    function tableInfo($result, $mode = null)
    {
        if (is_string($result)) {
             ...
             */
            $info = @$this->connection->query("PRAGMA table_info('$result');");
            $id = array();
