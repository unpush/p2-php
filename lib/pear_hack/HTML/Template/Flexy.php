...
class HTML_Template_Flexy
{
    ...
    */
    var $options = array(
        ...
        'useSubDirs'    => true,        // whether or not to use sub dirs in the compileDir.
                                        // if set to false, DIRECTORY_SEPARATOR will be replaced with "^", like Smarty.
    );
    /**
    ...
    */
    function compile( $file )
    {
        ...
        if (!$this->options['useSubDirs']) {
            $escaped = str_replace('/', DIRECTORY_SEPARATOR, $file);   // mostly for Windows.
            $escaped = str_replace(array('^', DIRECTORY_SEPARATOR), array('^^', '^'), $escaped);
            if ($compileSuffix) {
                $base = $compileDest . $compileSuffix . '^' . $escaped;
            } else {
                $base = $compileDest . DIRECTORY_SEPARATOR . $escaped;
            }
        } else {
