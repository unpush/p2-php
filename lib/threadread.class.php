...
class ThreadRead extends Thread{
    ...
    function downloadDat()
    {
        ...
        if (file_exists($this->keydat) && $_conf['read_dl_interval'] > 0 &&
            ($_conf['read_dl_interval'] + filemtime($this->keydat)) > time()
        ) {
            return;
        }
        ...
        // 2ch系
            ...
            if (P2Util::isHost2chs($this->host) && !empty($_GET['maru'])) {
                ...
                if (!file_exists($_conf['sid2ch_php']) || !empty($_REQUEST['relogin2ch']) ||
    ...
    function downloadDat2ch($from_bytes)
    {
        ...
        $method = 'GET';
        if (!isset($uaMona)) { $uaMona = 'Monazilla/1.00'; }
        $p2ua = $uaMona.' ('.$_conf['p2name_ua'].'/'.$_conf['p2version_ua'].')';
        ...
        if ($_conf['proxy_use']) {
            ...
            $send_host = $purl['host'];
            $send_port = !empty($purl['port']) ? $purl['port'] : $default_port;
    ...
    function downloadDat2chMaru()
    {
        ...
        //$request .= "Accept-Language: ja, en\r\n";
        $request .= "User-Agent: ".$p2ua."\r\n";
        //$request .= "X-2ch-UA: ".$_conf['p2name_ua'].'/'.$_conf['p2version_ua']."\r\n";
    ...
    function get2chDatError()
    {
        ...
        $wap_ua->setAgent($_conf['p2name_ua'].'/'.$_conf['p2version_ua']); // ここは、"Monazilla/" をつけるとNG
    ...
    function previewOne()
    {
        ...
        if ($this->onthefly) {
            $this->datlines = array($first_line);
        } elseif (!isset($this->datlines)) {
            $this->readDat($this->keydat);
        }
        $aShowThread = &new ShowThreadPc($this);
        $body .= $aShowThread->transRes(1); // 1を表示
        unset($aShowThread);
    ...
    function lsToPoint()
    {
        ...
        // まとめ読みの表示数制限
        if (isset($GLOBALS['rnum_all_range']) && $GLOBALS['rnum_all_range'] > 0) {
    ...
    function readDat()
    {
        ...
        if (file_exists($this->keydat)) {
            if ($this->datlines = @file($this->keydat)) {
                ...
                // ・・・と、文字化けしたタイトルのスレッドで誤判定があったので、eucJP-winで固定
                if (P2Util::isHostBe2chNet($this->host)) {
            ...
            if (P2Util::isHostNoCacheData($this->host)) {
                @unlink($this->keydat);
            }
        } else {
            return false;
    ...
    function setIdCount($lines)
    {
        if ($lines) {
            foreach ($lines as $line) {
                $lar = explode('<>', $line);
                $idchars = '0-9a-zA-Z\/.+';
                $idregex = '/ID: ?(['.$idchars.']{10}|['.$idchars.']{8})([,.0O]|†)?/';
                if (preg_match($idregex, $lar[2], $matches)) {
                    $id = $matches[1];
