...
$_conf['p2name_ua'] = $_conf['p2name']; // ($_conf['p2name']) サーバ接続時のUAの名前部分
$_conf['p2version_ua'] = $_conf['p2expack']; // ($_conf['p2expack']) サーバ接続時のUAのバージョン部分
...
// ("http://www6.ocn.ne.jp/~mirv/2chmenu.html") // 2ch基本（古い？）
...
$_conf['sb_sort_ita'] = 'ikioi';    // ('ikioi') PC閲覧時、板表示のスレッド一覧でのデフォルトのソート指定
...
$_conf['read_dl_interval'] = 0; // (0) dat のキャッシュを更新せずに保持する時間 (秒)
...
$_conf['preview_thumbnail'] = 0;    // (0) 画像URLの先読みサムネイル (表示する:1, しない:0)
    // この設定よりもconf_user_ex.phpのイメージキャッシュの設定が優先される
$_conf['pre_thumb_limit'] = 10; // (10) PC閲覧時、画像URLの先読みサムネイルを一度に表示する制限数 (-1にすると無制限)
    // ImageCache2では新規に画像をダウンロードする制限数
$_conf['pre_thumb_limit_k'] = 5;    // (5) 携帯閲覧時、ImageCache2のインラインプレビューを一度に表示する制限数 (-1にすると無制限)
    // キャッシュの有無に関わらず制限される（携帯はメモリに制限があるので）
$_conf['pre_thumb_height'] = "32";  // ("32") 画像サムネイルの縦の大きさを指定（ピクセル）
$_conf['pre_thumb_width'] = "32";   // ("32") 画像サムネイルの横の大きさを指定（ピクセル）
$_conf['iframe_popup'] = 2; // (2) HTMLポップアップ（する:1, しない:0, pでする:2, 画像でする:3）
$_conf['iframe_popup_delay'] = 0.2; // (0.2) HTMLポップアップの表示遅延時間（秒）
$_conf['ext_win_target'] = "_blank";    // ("_blank") 外部サイト等へジャンプする時に開くウィンドウのターゲット名（同窓:"", 新窓:"_blank"）
...
$_conf['before_respointer_k'] = 0;  // (0) 携帯閲覧時、ポインタの何コ前のレスから表示する
...
$_conf['through_ime'] = "ex";   // ("ex") 外部URLジャンプする際に通すゲート。（直接:"", p2 ime(自動転送):"p2", p2 ime(手動転送):"p2m", p2 ime(pのみ手動転送):"p2pm", moonshime:"ex"）
$_conf['motothre_ime'] = 0; // (0) 元スレへのリンクもimeを通す（する:1, しない:0）
...
$_conf['frame_type']=0;		// (0) frame分割形式(従来:0 その他:1-3)
$_conf['frame_cols']="156,*";	// ("156,*") 横方向の分割パラメータ
$_conf['frame_rows']="40%,60%";	// ("40%,60%") 縦方向の分割パラメータ
