...
function deleMsg($checked_hists)
{
    ...
    // 読み込んで
    if (!$reslines = DataPhp::fileDataPhp($_conf['p2_res_hist_dat_php'])) {
        die("p2 Error: {$_conf['p2_res_hist_dat_php']} を開けませんでした");
    }
    ...
    if (is_array($neolines)) {
        ...
        // 書き込み処理
        DataPhp::writeDataPhp($cont, $_conf['p2_res_hist_dat_php']);
    }
}
