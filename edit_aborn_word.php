...
if (!empty($_POST['submit_save'])) {
    ...
    if ($limit and count($newdata) > $limit) {
        ...
        P2Util::pushInfoHtml("<p><span style=\"color:red;\">注意: {$limit}個を超えるデータは登録できません。</span></p>");
    }
...
// プリント設定
//=====================================================================
$ptitle_top = sprintf('NG/あぼーんワード編集 &gt; <a href="%s?path=%s">%s</a>',
    $_SERVER['SCRIPT_NAME'], rawurlencode(basename($path)), basename($path));
