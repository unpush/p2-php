...
function _getMaruKakikoHtml($host)
{
    ...
    if (P2Util::isHost2chs($host) and file_exists($_conf['sid2ch_php'])) {
        $maru_kakiko_ht = sprintf(
            '<span>2ch●ID: <input id="maru_kakiko" name="maru_kakiko" type="checkbox" value="1"%s></span>',
...
function _getSrcFixHtml($host)
{
    ...
    if (UA::isPC() || UA::isIPhoneGroup()) {
        if (
            ...
            $src_fix_ht = '<input type="checkbox" id="fix_source" name="fix_source" value="1">ソースコード補正';
        }
    }
    return $src_fix_ht;
}
