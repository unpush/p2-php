...
/* iPhone 未完成なので一時的にはずしてます
	// 同じ名前をフィルタ表示
	document.writeln('<a id="' + thread_id + '_same_name" href="#" target="' + spmTarget + '">同じ名前</a>');
	
	// 逆参照
	document.writeln('<a id="' + thread_id + '_ref_res" href="' + getSpmFilterUrl(aThread, 'rres', 'on') + '&b=i" target="' + spmTarget + '">逆参照</a>');
	*/
// iphoneこれにレス
...
function showSPM(aThread, resnum, resid, event, obj)
{
	...
	var spm_iphone_res = p2GetElementById(aThread.objName + "_iphone_res");
	if(spm_iphone_res) {
		spm_iphone_res.innerHTML = getSpmResPhone();
	}
	var spm_iphone_res = p2GetElementById(aThread.objName + "_iphone_quoteres");
	if(spm_iphone_res) {
		spm_iphone_res.innerHTML = getSpmQuoteResPhone();
	}
...
// @access  private only this file
// @return  string
function getSpmQuoteResPhone()
{
	var onc = "onclick=\"resQuoteMessage(spmResNum);\"";
	var har = "href=\"#footer\"";
	var url = "<a " + har + " " + onc + ">引用してレス</a>";
	return url;
}
...
//レスに対してレスするのにメッセージ内容を書き換える
//本来はpost_form.js に書くべき？
...
function resQuoteMessage(resNum) {
	if(true == isSafari()){
		...
		var quoteMsg = p2GetElementById("quote_msg" + spmResNum).innerText;
	}
	var msg = p2GetElementById('MESSAGE')
	// 改行マークを改行に戻しつつ、msgボックスに値をセット
