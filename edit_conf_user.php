...
echo getEditConfHtml('sb_sort_ita', '板表示のスレッド一覧でのデフォルトの並び順の指定');
echo getEditConfHtml('sort_zero_adjust', '並び順が新着の時の「既得なし」の「新着数ゼロ」に対する並び順の優先順位');
echo getEditConfHtml('cmp_dayres_midoku', '並び順が勢いの時に新着レスのあるスレを優先');
...
echo getEditConfHtml('quote_res_view', '引用レスを（ポップアップ）表示');
...
echo getEditConfHtml('pane_mode', '3ペイン画面のフレームの並べ方');
