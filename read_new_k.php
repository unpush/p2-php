...
// 変数
//==================================================================
$rnum_all_range = $_conf['k_rnum_range'];
...
if ($aThreadList->spmode) {
    $sb_ht =<<<EOP
        <a href="{$_conf['subject_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}&amp;spmode={$aThreadList->spmode}">{$ptitle_hd}</a>
EOP;
    $sb_ht_btm =<<<EOP
        <a {$_conf['accesskey']}="{$_conf['k_accesskey']['up']}" href="{$_conf['subject_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}&amp;spmode={$aThreadList->spmode}">{$_conf['k_accesskey']['up']}.{$ptitle_hd}</a>
EOP;
} else {
    $sb_ht =<<<EOP
        <a href="{$_conf['subject_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}">{$ptitle_hd}</a>
EOP;
    $sb_ht_btm =<<<EOP
        <a {$_conf['accesskey']}="{$_conf['k_accesskey']['up']}" href="{$_conf['subject_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}">{$_conf['k_accesskey']['up']}.{$ptitle_hd}</a>
EOP;
}
...
for ($x = 0; $x < $linesize; $x++) {
    ...
    // subject (not spmode)
        ...
        $aThread->bbs = $aThreadList->bbs;
        // 板別お気にスレ新着まとめ読み
        if ($onlyfav && !$fav_keys[$aThread->key]) {
            unset($aThread);
            continue;
        }
    }
...
EOP;
    ...
    if ($aThread->rescount) {
        ...
        include_once (P2_LIBRARY_DIR . '/showthread.class.php');    // HTML表示クラス
    ...
    $read_footer_navi_new = "<a href=\"{$_conf['read_php']}?host={$aThread->host}{$bbs_q}{$key_q}&amp;ls={$aThread->rescount}-&amp;nt={$newtime}#r{$aThread->rescount}\">新着ﾚｽの表示</a>";

    $dores_ht = <<<EOP
        <a href="post_form.php?host={$aThread->host}{$bbs_q}{$key_q}&amp;rc={$aThread->rescount}{$ttitle_en_q}">ﾚｽ</a>
EOP;

    // ツールバー部分HTML =======
    if ($spmode) {
        $toolbar_itaj_ht = <<<EOP
(<a href="{$_conf['subject_php']}?host={$aThread->host}{$bbs_q}{$key_q}">{$itaj_hd}</a>)
EOP;
    }
    $toolbar_right_ht .=<<<EOTOOLBAR
            <a href="info.php?host={$aThread->host}{$bbs_q}{$key_q}{$ttitle_en_q}">{$info_st}</a>
            <a href="info.php?host={$aThread->host}{$bbs_q}{$key_q}{$ttitle_en_q}&amp;dele=true">{$delete_st}</a>
            <a href="{$motothre_url_ime}">元ｽﾚ</a>
EOTOOLBAR;

    $read_footer_ht = <<<EOP
        <div id="ntt_bt{$newthre_num}" name="ntt_bt{$newthre_num}">
            $read_range_ht
            <a href="{$_conf['read_php']}?host={$aThread->host}{$bbs_q}{$key_q}&amp;offline=1&amp;rc={$aThread->rescount}#r{$aThread->rescount}">{$aThread->ttitle_hd}</a>{$toolbar_itaj_ht}
...
if (!isset($rnum_all_range) || $rnum_all_range > 0) {
    echo <<<EOP
    <div>
        {$sb_ht_btm}の<a href="{$_conf['read_new_k_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}&amp;spmode={$aThreadList->spmode}&amp;nt={$newtime}{$onlyfav_a}" {$_conf['accesskey']}="{$_conf['k_accesskey']['next']}">{$_conf['k_accesskey']['next']}.{$onlyfav_ht}新まとめを更新</a>
    </div>\n
EOP;
} else {
    echo <<<EOP
    <div>
        {$sb_ht_btm}の<a href="{$_conf['read_new_k_php']}?host={$aThreadList->host}&amp;bbs={$aThreadList->bbs}&amp;spmode={$aThreadList->spmode}&amp;nt={$newtime}&amp;norefresh=1{$onlyfav_a}" {$_conf['accesskey']}="{$_conf['k_accesskey']['next']}">{$_conf['k_accesskey']['next']}.{$onlyfav_ht}新まとめの続き</a>
    </div>\n
EOP;
}
